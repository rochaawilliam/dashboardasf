...
export function CircularProgressCard({
  ...
  return (
    <div
      className={cn('metric-card group relative min-w-0 overflow-hidden break-words p-2 sm:p-4 lg:p-4 h-full flex flex-col',
      ...
      <div className='mb-0.5 sm:mb-2 lg:mb-3 flex items-center gap-1.5 min-w-0'>
        <span className='metric-label text-sm sm:text-base lg:text-base font-semibold flex-1 min-w-0 leading-tight line-clamp-2 break-words'>{metric.name}</span>
        {dataSourceBadge && (
          <Tooltip>
      ...
      <div className='flex-1 flex items-stretch gap-2 sm:gap-4 lg:gap-4 min-w-0'>
          ...
          {resultadoData ? (
            <>
              <div className='min-w-0'>
                <p className='text-[10px] sm:text-[11px] lg:text-xs text-muted-foreground uppercase tracking-wide min-w-0 break-words'>
                  Previsto
                </p>
                <p className='font-semibold text-foreground leading-tight text-base sm:text-xl lg:text-2xl tracking-tighter min-w-0 break-words'>
              ...
              <div className='min-w-0'>
                ...
                <p className={cn(
                  'leading-tight text-xl sm:text-2xl lg:text-3xl font-sans font-extrabold tracking-tighter min-w-0 break-words',
            ...
            <>
              ...
              <div className='min-w-0'>
                ...
                <div className='flex items-center gap-1 min-w-0'>
                  <div className='flex flex-col min-w-0'>
                    <p className='text-foreground leading-tight text-lg sm:text-2xl lg:text-3xl font-sans font-extrabold tracking-tighter min-w-0 break-words'>
                      {hideValues ? '••••••' : (
                        <>
                          {formatMetricValue(displayValue, metric.unit, metric.name)}
                          {getStageLabel(metric.name, displayValue) && (
                            <span className='text-base sm:text-xl lg:text-2xl font-semibold text-muted-foreground ml-1 break-words'>
                              {getStageLabel(metric.name, displayValue)}
                            </span>
                          )}
                        </>
                      ...
                      <TooltipContent side='bottom' className='text-xs max-w-[320px] max-h-[300px] overflow-y-auto'>
                        ...
                        <ul className='space-y-0.5'>
                          {pipelineCardNames.map((name, i) => (
                            <li key={i} className='text-popover-foreground/80 break-words'>• {name}</li>
                          ))}
                        </ul>
                      </TooltipContent>
                    </Tooltip>
              ...
              {!hideTarget && isMonthSelected && !isNonAccumulative && !forceAnnualLabel && !hideAnnualTarget && (
                <div className='text-[7px] sm:text-[8px] text-muted-foreground mt-1 flex items-center gap-1 min-w-0'>
                  <span className='shrink-0'>Meta anual:</span>
                  <span className='font-medium truncate min-w-0'>{hideValues ? '••••••' : formatMetricValue(annualTarget, metric.unit, metric.name)}</span>
                </div>
              )}
              ...
              {metric.description && (metric.description.startsWith('Projeção') || metric.description.startsWith('Meta anual')) && (
                <p className='text-[10px] sm:text-xs lg:text-xs text-muted-foreground mt-0 font-medium min-w-0 break-words'>
                  📊 {metric.description}
                </p>
              )}
            </>
        ...
        {sideContent && (
          <div className='shrink-0 basis-[38%] sm:basis-[34%] lg:basis-[32%] max-w-[9.5rem] min-w-0 flex flex-col justify-center gap-1 border-l border-border/50 pl-1.5 sm:pl-3 break-words overflow-hidden'>
            {sideContent}
          </div>
        )}
