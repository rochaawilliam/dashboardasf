...
export function MetricCardMonthly({ 
  ...
  return (
    <>
      <div 
        className={cn(
          'metric-card group relative min-w-0 overflow-hidden break-words p-1.5 sm:p-2 lg:p-2.5 border-l-[3px]',
      ...
      >
        ...
        <div className='flex items-start justify-between gap-1 mb-1 sm:mb-1.5'>
          <div className='flex-1 min-w-0 flex items-center gap-1.5'>
            <span className='metric-label text-[9px] sm:text-[10px] lg:text-[11px] font-medium line-clamp-2 min-w-0 break-words'>{metric.name}</span>
            {metric.description && (
              <Tooltip>
        ...
        {isEditing ? (
          <div className='space-y-2' onClick={(e) => e.stopPropagation()}>
            <div className='flex items-center gap-2'>
              <Input
                ...
                autoFocus
              />
              {metric.unit !== 'número' && <span className='text-[10px] text-muted-foreground shrink-0'>{metric.unit}</span>}
            </div>
          ...
          <>
            ...
            <div className='flex items-end justify-between gap-1 mb-1 sm:mb-1.5'>
              ...
              <div className='flex-1 min-w-0'>
                {hasNoData ? (
                  ...
                  <div className='text-[10px] sm:text-sm lg:text-lg font-bold leading-tight text-primary truncate tracking-tighter max-w-full'>
              ...
              <div className='text-right shrink-0 min-w-0 max-w-[48%] sm:max-w-[46%] lg:max-w-[48%]'>
                {isMonthSelected ? (
                  <>
            ...
            {isMonthSelected && !isNonAccumulative && (
              <div className='text-[7px] sm:text-[8px] text-muted-foreground mb-1 sm:mb-1.5 flex items-center gap-1 min-w-0'>
                <span className='shrink-0'>Meta anual:</span>
                <span className='font-medium truncate min-w-0'>{formatMetricValue(metric.target_value, metric.unit, metric.name)}</span>
              </div>
            )}
            ...
            <div className='flex items-stretch gap-1 lg:gap-1.5 mt-1 min-w-0'>
              <Sparkline
                metricId={metric.id}
